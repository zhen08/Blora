...
RHGenericDriver::RHGenericDriver()
    : _mode(RHModeInitialising), _thisAddress(RH_BROADCAST_ADDRESS),
      ...
      _txHeaderId(0), _txHeaderFlags(0), _rxBad(0), _rxGood(0), _txGood(0),
      _cad_timeout(0) {}
...
void RHGenericDriver::waitAvailable() {
  while (!available())
    Serial.println("RHGenericDriver::waitAvailable");
  YIELD;
}
...
bool RHGenericDriver::waitAvailableTimeout(uint16_t timeout) {
  ...
  while ((millis() - starttime) < timeout) {
    if (available()) {
      Serial.println("RHGenericDriver::waitAvailableTimeout");
      return true;
    }
    YIELD;
  }
...
void RHGenericDriver::setHeaderFrom(uint8_t from) { _txHeaderFrom = from; }

void RHGenericDriver::setHeaderId(uint8_t id) { _txHeaderId = id; }
...
uint8_t RHGenericDriver::headerTo() { return _rxHeaderTo; }

uint8_t RHGenericDriver::headerFrom() { return _rxHeaderFrom; }

uint8_t RHGenericDriver::headerId() { return _rxHeaderId; }

uint8_t RHGenericDriver::headerFlags() { return _rxHeaderFlags; }

int8_t RHGenericDriver::lastRssi() { return _lastRssi; }
