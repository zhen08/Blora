...
// RHDatagram will be received only by the addressed node or all nodes within
// range if the
...
uint8_t RHMesh::sendtoWait(uint8_t *buf, uint8_t len, uint8_t address,
  ...
  // Now have a route. Contruct an application layer message and send it via
  // that route
  MeshApplicationMessage *a = (MeshApplicationMessage *)&_tmpMessage;
  a->header.msgType = RH_MESH_MESSAGE_TYPE_APPLICATION;
  memcpy(a->data, buf, len);
  return RHRouter::sendtoWait(
      _tmpMessage, sizeof(RHMesh::MeshMessageHeader) + len, address, flags);
}

void RHMesh::setArpTimeout(unsigned long value) { arpTimeout = value; }
////////////////////////////////////////////////////////////////////
bool RHMesh::doArp(uint8_t address) {
  ...
  uint8_t error =
      RHRouter::sendtoWait((uint8_t *)p, sizeof(RHMesh::MeshMessageHeader) + 2,
...
void RHMesh::peekAtMessage(RoutedMessage *message, uint8_t messageLen) {
  ...
  if (messageLen > 1 &&
    ...
    // being routed back to the originator here. Want to scrape some routing
    // data out of the response
    // We can find the routes to all the nodes between here and the responding
    // node
    MeshRouteDiscoveryMessage *d = (MeshRouteDiscoveryMessage *)message->data;
    addRouteTo(d->dest, headerFrom());
    uint8_t numRoutes = messageLen - sizeof(RoutedMessageHeader) -
                        sizeof(MeshMessageHeader) - 2;
    uint8_t i;
    // Find us in the list of nodes that were traversed to get to the responding
    // node
...
uint8_t RHMesh::route(RoutedMessage *message, uint8_t messageLen) {
  uint8_t from =
      headerFrom(); // Might get clobbered during call to superclass route()
  ...
  if (ret == RH_ROUTER_ERROR_NO_ROUTE ||
    ...
    if (message->header.source != _thisAddress) {
      ...
      // Make sure there is a route back towards whoever sent the original
      // message
      addRouteTo(message->header.source, from);
      ret = RHRouter::sendtoWait((uint8_t *)p,
                                 sizeof(RHMesh::MeshMessageHeader) + 1,
...
bool RHMesh::isPhysicalAddress(uint8_t *address, uint8_t addresslen) {
  // Can only handle physical addresses 1 octet long, which is the physical node
  // address
  return addresslen == 1 && address[0] == _thisAddress;
}

////////////////////////////////////////////////////////////////////
bool RHMesh::recvfromAck(uint8_t *buf, uint8_t *len, uint8_t *source,
  ...
  uint8_t _id;
  uint8_t _flags;
  if (RHRouter::recvfromAck(_tmpMessage, &tmpMessageLen, &_source, &_dest, &_id,
                            &_flags)) {
    MeshMessageHeader *p = (MeshMessageHeader *)&_tmpMessage;

  Serial.print("RHMesh::recvfromAck");
  Serial.println(p->msgType);
    if (tmpMessageLen >= 1 && p->msgType == RH_MESH_MESSAGE_TYPE_APPLICATION) {
      ...
      return true;
    } else if (_dest == RH_BROADCAST_ADDRESS && tmpMessageLen > 1 &&
      ...
      // Message is an array of node addresses the route request has already
      // passed through
      ...
      if (isPhysicalAddress(&d->dest, d->destlen)) {
        // This route discovery is for us. Unicast the whole route back to the
        // originator
        ...
        // Its for someone else, rebroadcast it, after adding ourselves to the
        // list
        d->route[numRoutes] = _thisAddress;
        tmpMessageLen++;
        // Have to impersonate the source
        // REVISIT: if this fails what can we do?
        RHRouter::sendtoFromSourceWait(_tmpMessage, tmpMessageLen,
                                       RH_BROADCAST_ADDRESS, _source);
      }
    }
  }
  return false;
...
////////////////////////////////////////////////////////////////////
bool RHMesh::recvfromAckTimeout(uint8_t *buf, uint8_t *len, uint16_t timeout,
                                uint8_t *from, uint8_t *to, uint8_t *id,
                                uint8_t *flags) {
  unsigned long starttime = millis();
  int32_t timeLeft;
